...
from speech2gesture.anim.tquat import *
from speech2gesture.anim.txform import xform_orthogonalize_from_xy
...
class Decoder(nn.Module):
    ...
    def open_streaming_mode(self, names, parents):
        self.streaming_mode = True
        self.names = names
        self.parents = parents

    def close_streaming_mode(self):
        self.streaming_mode = False
    ...
    def forward(
        ...
        self.offset_pos = Z_root_pos.detach().cpu().numpy().copy()[:None]
        self.offset_rot = Z_root_rot.detach().cpu().numpy().copy()[:None]

        self.start_position = np.array([0, 0, 0])
        self.start_rotation = np.array([1, 0, 0, 0])

        V_root_pos = Z_root_pos[:, None]
        V_root_rot = Z_root_rot[:, None]
        V_root_vel = Z_root_vel[:, None]
        V_root_vrt = Z_root_vrt[:, None]
        V_lpos = Z_lpos[:, None]
        V_ltxy = Z_ltxy[:, None]
        V_lvel = Z_lvel[:, None]
        V_lvrt = Z_lvrt[:, None]
        ...
        if self.verbose:
            print(f"State Encoder time: {time() - start:.4f}")
        ...
        for i in range(1, nframes):
            ...
            V_root_pos = torch.cat([V_root_pos, P_root_pos[:, None]], dim=1)
            V_root_rot = torch.cat([V_root_rot, P_root_rot[:, None]], dim=1)
            V_root_vel = torch.cat([V_root_vel, P_root_vel[:, None]], dim=1)
            V_root_vrt = torch.cat([V_root_vrt, P_root_vrt[:, None]], dim=1)
            V_lpos = torch.cat([V_lpos, P_lpos[:, None]], dim=1)
            V_ltxy = torch.cat([V_ltxy, P_ltxy[:, None]], dim=1)
            V_lvel = torch.cat([V_lvel, P_lvel[:, None]], dim=1)
            V_lvrt = torch.cat([V_lvrt, P_lvrt[:, None]], dim=1)

            if self.verbose:
                print(f"Frame {i} time {time() - start:.4f}")

            if self.streaming_mode:
                start = time()
                frame_V_lrot = quat.from_xform(
                    xform_orthogonalize_from_xy(P_ltxy[:, None]).detach().cpu().numpy())[0]

                frame_V_root_pos = P_root_pos.detach().cpu().numpy()[:None]
                frame_V_root_rot = P_root_rot.detach().cpu().numpy()[:None]
                frame_V_lpos = P_lpos.detach().cpu().numpy()[:None]

                inv_rot = quat.inv(self.offset_rot)

                frame_V_root_pos = quat.mul_vec(inv_rot, frame_V_root_pos - self.offset_pos)
                frame_V_root_rot = quat.mul(inv_rot, frame_V_root_rot)
                frame_V_root_pos = (
                        quat.mul_vec(self.start_rotation[np.newaxis], frame_V_root_pos) + self.start_position[
                    np.newaxis]
                )
                frame_V_root_rot = quat.mul(self.start_rotation[np.newaxis], frame_V_root_rot)

                frame_V_lpos[:, 0] = quat.mul_vec(frame_V_root_rot, frame_V_lpos[:, 0]) + frame_V_root_pos
                frame_V_lrot[:, 0] = quat.mul(frame_V_root_rot, frame_V_lrot[:, 0])

                # np.save(f"./data/frame{i}-pos.npy", frame_V_lpos)
                # np.save(f"./data/frame{i}-rot.npy", frame_V_lrot)

                yield self.frame_data_to_json(frame_V_lpos, frame_V_lrot)

        yield (
            V_root_pos, V_root_rot, V_root_vel, V_root_vrt, V_lpos, V_ltxy, V_lvel, V_lvrt
        )

    def frame_data_to_json(self, frame_V_lpos, frame_V_lrot):
        frame_data = {"MetaHuman": []}
        for idx, bone_name in enumerate(self.names):
            frame_data["MetaHuman"].append(
                {"Name": bone_name, "Parent": int(self.parents[idx]), "Location": frame_V_lpos[0][idx].tolist(),
                 "Rotation": np.degrees(quat.to_euler(frame_V_lrot[0][idx], "zyx")).tolist(), "Scale": [1.0, 1.0, 1.0]})
...
class CellStateEncoder(nn.Module):
    ...
    def forward(self, pose, style):
        ...
        return output.reshape(output.shape[0], self.num_rnn_layers, -1).transpose(0, 1).contiguous()
