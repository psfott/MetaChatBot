...
def convert_to_desired(file_path, desired_fs=None, desired_nb_channels=None, out_type='float32'):
    try:
        ...
        old_str = ".wav" if ".wav" in file_path else ".WAV"
        tmp_wav = os.path.join(os.path.dirname(file_path),
                               os.path.basename(file_path).replace(old_str, '_tmp.wav'))
        ...
        # reformat
        print(f'{file_path} -- {e}')
        print(f'converting to {desired_fs}Hz/{desired_bit_depth} bits/{desired_nb_channels} channels')
        ...
        reformat_and_trim_wav_file(file_path, fs=desired_fs, bit_depth=desired_bit_depth,
                                   ...
                                   silence_threshold=-1., silence_pad=False, logger=None)

        os.remove(file_path)
        os.rename(tmp_wav, file_path.replace(".WAV", ".wav"))
...
def read_wavfile(file_path, rescale=False, desired_fs=None, desired_nb_channels=None, out_type='float32', logger=None,
    ...
    # cast to desired pre-generate type
